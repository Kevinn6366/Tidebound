...
// ==========================================
// 【已并入主程序】剧情IDE（原 GWC-剧情IDE拓展包）
// 原为独立插件，现为程序内置功能模块：随主包编译，不再出现在插件管理中，
// 也不再从 /app/mods/ 单独下载。由 AppCore 在插件初始化阶段调用 installScriptMode()。
// 内部实现保持原样（已验证无隐式全局/顶层 this，可安全运行于 ES 模块严格模式）。
// ==========================================
...
export function installScriptMode() {
    if (window.__ScriptModeDLCLoaded_V4_35) return;
    window.__ScriptModeDLCLoaded_V4_35 = true;

    // 【修复】本模块有 6 处引用 API_BASE（场景/CG/音乐的上传与背景读取），
    // 但它其实定义在 _gwc_api_client.js 的 IIFE 内部，从来不是全局变量，
    // 因此这些调用一直在抛 ReferenceError —— 且都被 try/catch 与 .catch() 吞掉，
    // 表现为“上传无效、读不到已有文件”。前后端同源，故与 api.js/db.js 保持一致取空串。
    const API_BASE = '';
  ...
  console.log("[Script Mode DLC] V4.35 究极导演版 开始装载...");
    ...
    const injectGlobalStyles = () => {
        ...
        style.innerHTML = `
           ...
           /* 新增图片悬浮预览窗 */
            ...
            /* 顶栏折行规则已统一到文末的“分辨率适配与控件规范”区块，
               不再使用依赖完整 class 串的脆弱选择器（class 一变就失效）。 */
            ...
            /* ==========================================================
               分辨率适配与控件规范（统一收口，避免按钮/输入框重叠、风格不一）
               ========================================================== */

            /* 顶栏：三组元素弹性排布并允许折行。
               原先中间的视图切换是 absolute 居中，不参与 flex 布局，
               屏幕变窄时会与左右两侧直接重叠 —— 已改为普通 flex 项。 */
            #sm-editor-modal .sme-view-toggle { position: static !important; transform: none !important; flex: 0 0 auto; }
            #sm-editor-modal > .h-16,
            #sm-editor-modal .h-16 {
            ...
            /* 顶栏内的分组本身也要能收缩换行，避免撑破容器 */
            #sm-editor-modal .h-16 > div { flex-wrap: wrap; gap: .5rem; min-width: 0; }

            /* 剧本名输入框：原为固定 w-48，窄屏时挤压相邻按钮 */
            #sm-editor-modal #sme-proj-name { width: clamp(7rem, 18vw, 14rem); min-width: 7rem; }

            /* 控件统一规范：高度、圆角、字号一致，且不因文字换行而互相错位。
               原代码里圆角混用 rounded / rounded-lg / rounded-full，
               内边距有 7 种规格，同一排按钮观感参差 —— 这里统一收口，
               但不动各按钮自身的语义配色（保留“危险/确认/普通”的区分）。 */
            #sm-editor-modal button {
                white-space: nowrap;      /* 按钮文字不折行，避免高度参差导致错位 */
                flex-shrink: 0;           /* 空间不足时不被压扁成重叠 */
                line-height: 1.25;
                border-radius: .5rem;     /* 统一圆角 */
                min-height: 2rem;         /* 统一最小高度，同排按钮基线一致 */
                display: inline-flex; align-items: center; justify-content: center;
                gap: .25rem;
            }
            /* 统一内边距：仅规范未显式指定的与常见的几档，圆形图标按钮除外 */
            #sm-editor-modal button.rounded-full { border-radius: 9999px; }
            #sm-editor-modal .h-16 button { padding: .375rem .875rem; font-size: .8125rem; }
            /* 删除类小图标按钮保持紧凑，不被上面的规则撑大 */
            #sm-editor-modal button.sme-btn-del,
            #sm-editor-modal button[class*="text-[10px]"],
            #sm-editor-modal button[class*="text-xs"].px-2 { padding: .125rem .5rem; min-height: 1.5rem; }
            #sm-editor-modal select,
            #sm-editor-modal input[type="text"],
            #sm-editor-modal input[type="number"] {
                min-width: 0;             /* 允许在 flex/grid 中正常收缩，防止溢出遮挡 */
                max-width: 100%;
                box-sizing: border-box;
            }
            /* 行内资源选择器（背景/CG/音乐）成组换行，不再横向溢出 */
            #sm-editor-modal .sme-line-item .flex { flex-wrap: wrap; }

            /* 各弹窗一律不超出视口，内部滚动 */
            #sm-editor-modal, #sm-story-selector, #sm-story-save-ui, #sm-player-map-overlay { max-width: 100vw; max-height: 100vh; }
            #sm-story-selector > div, #sm-story-save-ui > div {
                max-width: min(96vw, 1100px);
                max-height: 92vh;
                overflow-y: auto;
            }

            /* 中等屏幕（笔记本 1366x768 等）：压缩留白，保证纵向可用高度 */
            @media (max-width: 1400px) {
                #sm-editor-modal .p-8 { padding: 1.25rem !important; }
                #sm-editor-modal .px-6 { padding-left: 1rem !important; padding-right: 1rem !important; }
                #sm-editor-modal .gap-4 { gap: .75rem !important; }
            }
            @media (max-width: 1200px) {
                #sm-editor-modal .w-80 { width: 18rem !important; }
                #sm-editor-modal .w-64 { width: 14rem !important; }
                #sm-editor-modal .text-lg { font-size: 1rem !important; }
            }
            /* 平板/窄窗口：侧栏改为上下堆叠，彻底避免三栏挤压重叠 */
            @media (max-width: 1024px) {
                /* 只针对三栏容器；主列也带 flex/flex-1/overflow-hidden，需用 :not(.flex-col) 排除，
                   否则会给主列多加一层滚动容器 */
                #sm-editor-modal .flex.flex-1.overflow-hidden:not(.flex-col) { flex-direction: column !important; overflow-y: auto !important; }
                #sm-editor-modal .w-64, #sm-editor-modal .w-80 {
                    width: 100% !important;
                    border-left: none !important; border-right: none !important;
                    border-bottom: 2px solid #e6d5b8;
                    max-height: 32vh; overflow-y: auto;
                }
                #sm-editor-modal .sme-view-toggle { order: 3; width: 100%; justify-content: center; }
            }
        `;
    ...
    // ================== 此处替换原有的预览方法 ==================
    // id -> 可播放 URL 的缓存。音乐分散在两个存储区，播放时需要据此解析。
    const bgmUrlCache = {};

    // 获取全局BGM音乐（服务端 API）
    // 【修复】音乐此前读不到：本函数只查插件 audio 存储区，而快速上传写入的是
    // 主程序全局 BGM 库（/api/userdata/{mid}/bgm），两者不是同一个地方。
    // 现在两个来源都读取并合并（与 getMainBgs 读取主背景库的策略一致）。
    const getBgms = async () => {
        ...
        if (api) {
            try {
                ...
                items.filter(a => a.id && a.id.toLowerCase().includes('bgm')).forEach(a => {
                    const url = api.getPluginBlobUrl('audio', a.id);
                    bgmUrlCache[a.id] = url;
                    out.push({ ...a, data: url });
                });
            } catch { /* 忽略，继续读主库 */ }
        }
        // 2) 主程序全局 BGM 库（快速上传写入的就是这里，也含用户在主程序添加的音乐）
        try {
            const resp = await fetch(`${API_BASE}/api/userdata/${mid}/bgm`);
            if (resp.ok) {
                const list = await resp.json();
                (list || []).forEach(b => {
                    if (!b || !b.id || bgmUrlCache[b.id]) return;
                    const url = `${API_BASE}/api/userdata/${mid}/bgm/${encodeURIComponent(b.id)}/file`;
                    bgmUrlCache[b.id] = url;
                    out.push({ id: b.id, name: b.name || b.id, data: url });
                });
            }
        } catch { /* 主库不可用时至少返回插件区结果 */ }
        return out;
    };
    ...
    /** 提示未接入的配音后台，避免原按钮调用不存在的函数。 */
    const doTTSForSingleLine = async () => { window.$GWC.showToast('配音后端尚未接入', 'info'); };
    const doTTSForSpeaker = doTTSForSingleLine;

    const askAIText = async () => { throw new Error('剧本生成后端尚未接入，手工编辑和播放功能保留'); };

    const generateScriptViaAI = async () => { throw new Error('剧本生成后端尚未接入，手工编辑和播放功能保留'); };
        ...
        const renderEditor = (resetScroll = false) => {
          ...
          const activeChapter = project.chapters[activeChapterIdx];
           ...
           const currentLabels = (activeChapter.lines || []).filter(l => l.type === 'label').map(l => ({id: l.labelId, name: l.labelName || '未命名节点'}));
            ...
            const bgmOptions = bgms.map(b => `<option value="${b.id}">🎵 ${b.name || b.id}</option>`).join('');
            ...
            const headerHtml = `
                <div style="background-color: #efe6d5; border-bottom: 1px solid #d9c5b2;" class="h-16 flex items-center justify-between px-6 shrink-0 z-20">
                    ...
                    <div style="background-color: rgba(217,197,178,0.4); display: flex;" class="sme-view-toggle items-center p-1 rounded-lg shadow-inner">
                    ...
                    chapterContentHtml = `
                        <div class="flex items-center gap-2 mt-2">
                            ...
                            <select id="sme-chap-bgm" class="sme-chap-bgm bg-white border border-slate-300 rounded px-2 py-1 text-xs outline-none text-emerald-600 font-bold">
                                ...
                                ${bgms.map(b => `<option value="${b.id}" ${activeChapter.chapBgm === b.id ? 'selected' : ''}>🎵 ${b.name || b.id}</option>`).join('')}
                            </select>
                        </div>
                                              ...
                                              <div style="border: 1px solid #d9c5b2; background-color: #fdfaf5;" class="flex-[1.5] min-w-[120px] flex items-stretch rounded overflow-hidden shadow-sm">
                                                ...
                                                <div style="border: 1px solid #d9c5b2; background-color: #fdfaf5;" class="flex-[1.5] min-w-[120px] flex items-stretch rounded overflow-hidden shadow-sm">
                                                    ...
                                                    <select class="sme-line-bgm flex-1 border-none outline-none text-[10px] font-bold bg-transparent pl-2 pr-1 py-1.5 cursor-pointer" style="color: #10b981;" data-lidx="${lIdx}">
                                                        ...
                                                        ${bgms.map(b => `<option value="${b.id}" ${line.bgm === b.id ? 'selected' : ''}>🎵 ${b.name || b.id}</option>`).join('')}
                                                    </select>
                                                </div>
...
const resolveTargetCh = (target, currentIdx) => {
    ...
    // ✨ 自动拦截并触发主角自定义名称替换（在内存层面执行，绝对不会污染原始工程数据）
        ...
        const playBgm = async (bgmId) => {
            ...
            const api = window.__GWC_API;
            // 【修复】音乐可能存在于插件 audio 区或主程序 BGM 库，
            // 缓存未命中时先拉取一次列表把两个来源都解析出来。
            let url = bgmUrlCache[bgmId];
            if (!url) { try { await getBgms(); } catch {} url = bgmUrlCache[bgmId]; }
            if (!url && api) url = api.getPluginBlobUrl('audio', bgmId);
            if (!url) return;
