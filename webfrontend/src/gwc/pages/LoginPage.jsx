...
export default function LoginPage({ onLogin, loginConfig }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isRegister) {
        const result = await registerUser(username.trim(), password, localMode);
        if (result.ok) {
          onLogin();
        } else {
          setError(result.msg);
        ...
        const result = await loginUser(username.trim(), localMode ? '' : password, rememberMe);
        if (result.ok) {
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        ...
        <div className="rounded-2xl p-8 shadow-2xl" style={{
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {localMode && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold" style={{
                background: 'rgba(90,180,237,0.12)', border: '1px solid rgba(90,180,237,0.25)', color: '#7dd3fc'
              }}>
                <Key size={14} /> 本地模式已开启：无需密码即可登录
              </div>
            )}
