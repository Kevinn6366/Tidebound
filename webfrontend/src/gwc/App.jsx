...
import { loadAnimRate, applyAnimRate, initClickSound } from './utils/uiFeedback';
...
export default function App() {
  ...
  // ✨ 全局 UI 反馈在登录页也要生效（动画速率 + 点击音效监听提前挂载）
  useEffect(() => {
    applyAnimRate(loadAnimRate());
    initClickSound(() => {
      try {
        const enabled = localStorage.getItem('gwc_click_sound_on') === '1';
        const url = localStorage.getItem('gwc_click_sound_url') || '';
        const volume = Number(localStorage.getItem('gwc_click_sound_vol')) || 0.5;
        return { enabled, url, volume };
      } catch { return { enabled: false, url: '', volume: 0.5 }; }
    });
  }, []);
  ...
  return (
    <div key="app-main" className="gwc-page-reveal">
