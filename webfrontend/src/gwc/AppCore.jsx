...
function OCPanel({ taskId, setOcTaskId, settings, setSettings, visible, setOcVisible }) {
  ...
  return (
    <div ref={panelRef} onMouseDown={onDown} className="oc-panel gwc-slide-up"
...
import { applyAnimRate, loadAnimRate } from './utils/uiFeedback';
...
export default function AppCore({ router }) {
  ...
  // ✨ 全局 UI 反馈：动画速率（启动即生效，不等待数据加载）
  useEffect(() => { applyAnimRate(loadAnimRate()); }, []);
  useEffect(() => { if (settings.animationRate) applyAnimRate(settings.animationRate); }, [settings.animationRate]);
  // 点击音效配置同步到 localStorage（全局监听在 App.jsx 挂载，统一从这里读取）
  useEffect(() => {
    try {
      localStorage.setItem('gwc_click_sound_on', settings.uiClickSoundEnabled ? '1' : '0');
      localStorage.setItem('gwc_click_sound_url', settings.uiClickSoundUrl || '');
      localStorage.setItem('gwc_click_sound_vol', String(settings.uiClickSoundVolume ?? 0.5));
    } catch {}
  }, [settings.uiClickSoundEnabled, settings.uiClickSoundUrl, settings.uiClickSoundVolume]);
  ...
  useEffect(() => {
    const loadEverything = async () => {
      try {
        ...
        // 加载失败：绝不用空数据覆盖服务端，也不新建空会话回写
        loadOkRef.current = false;
        if (e && (e.status === 401 || e.status === 403)) {
          // 登录已失效：清除会话并返回登录页（token 已清除，不会循环）
        ...
        setTimeout(() => {
          if (window.$GWC && window.$GWC.showToast) {
            window.$GWC.showToast("⚠️ 无法连接后端，数据未加载。请检查后端服务后刷新（避免直接操作，以防覆盖存档）。", "error", 10000);
          }
        }, 1500);
  ...
  useEffect(() => { if (canWrite()) saveCoreData('live2d_memos_v35', memos).catch(error => showToast(`保存失败：${error.message}`, 'error')); }, [memos]);
...
// ✨ 核心大迁徙修复：当底层数据库(IDB)把设置唤醒完毕后，强制“踢”一脚触发模型重载！
 ...
 const [isFaceTrackingLoading, setIsFaceTrackingLoading] = useState(false);
  ...
  useEffect(() => {
    const initMods = async () => {
        try {
            ...
            // 老用户的存档里仍有这些记录，若不清理会在插件管理中留下
            // 既无法工作、也无法卸载的空壳条目。
            const legacy = list.filter(m => MERGED_INTO_CORE_IDS.includes(m.id));
            if (legacy.length > 0) {
                list = list.filter(m => !MERGED_INTO_CORE_IDS.includes(m.id));
                legacy.forEach(m => {
                    deleteModFromDB(m.id).catch(() => {});
            ...
            const waitFor = async (test, timeout = 8000, step = 50) => {
                const t0 = Date.now();
                while (!test()) {
            ...
            const depsReady = await waitFor(() => window.$GWC && window.__GWC_API);
            if (!depsReady) console.warn('[内置功能] 依赖未就绪（$GWC/__GWC_API），仍尝试启动');
            // 两者互不依赖，任一失败不影响另一个及后续插件加载
  ...
  const processBridgeTask = useCallback(async () => { showToast("桌宠桥接尚未接入", "info"); }, [showToast]);

  useEffect(() => { /* 对应后端执行能力尚未接入，保留界面设置，不启动轮询。 */ }, []);
 ...
 const handleSkip = useCallback((e) => { e.stopPropagation(); if (pages.length > 0) { setVnPage(pages.length - 1); } }, [pages.length]);
  ...
  const loadScripts = async () => {
    ...
    const backupModule = window.module; const backupExports = window.exports; window.module = undefined; window.exports = undefined;
    try {
      // 阶段1：cubism core（live2d 框架与 pixi-live2d-display 都依赖它，必须先就绪）
      const offlineCore = await loadCoreData('offline_cubism_core_js');
      if (offlineCore) {
      ...
      // 阶段2：pixi.js（渲染器，独立）
      await injectScriptWithFallback('/app/vendor/pixi.min.js?v=2');

      // 阶段3：live2d 框架（pixi-live2d-display 依赖 Live2D 全局，必须在其之前加载，
      //        否则并行加载会出现 pixi-live2d-display 先初始化、找不到 Live2D 导致挂载失败）
      await injectScriptWithFallback('/app/vendor/live2d.min.js?v=2');

      // 阶段4：pixi-live2d-display（依赖 Live2D 与 Cubism Core，均已就绪）
      await injectScriptWithFallback('/app/vendor/pixi-live2d-display.min.js?v=2');
      ...
      setLive2dStatus('引擎库加载失败: ' + error.message);
      return false;
    } finally {
      if (backupModule !== undefined) window.module = backupModule;
      if (backupExports !== undefined) window.exports = backupExports;
  ...
  // 移除已加载但可能初始化失败的 Live2D 相关脚本标签，供重试时重新注入
  const removeLive2dScriptTags = () => {
    try {
      document.querySelectorAll('script[src*="pixi.min.js"], script[src*="live2d.min.js"], script[src*="pixi-live2d-display.min.js"], script[src*="Live2DCubismCore.js"]').forEach(s => s.remove());
    } catch (e) {}
  };
  useEffect(() => {
    let isMounted = true;
    const initLive2D = async () => {
      ...
      setLive2dStatus('加载引擎中...');
      let scriptsLoaded = await loadScripts();
      if (!isMounted) return;
      if (!scriptsLoaded) {
        // 首次失败：清掉可能「已加载但初始化失败」的脚本标签，重试一次
        removeLive2dScriptTags();
        setLive2dStatus('引擎加载失败，正在重试...');
        await new Promise(r => setTimeout(r, 300));
        scriptsLoaded = await loadScripts();
        if (!isMounted) return;
      }
      if (!scriptsLoaded) return setLive2dStatus('引擎加载失败，请刷新页面重试');
 ...
 const handleModelUpload = async (e) => {
  ...
  const triggerMemoryCompression = async () => { showToast("记忆整理接口尚未接入", "info"); };

 const generatePlotOptions = async () => { showToast("剧情选项接口尚未接入", "info"); };
 ...
 const clearTTSQueue = useCallback(() => {
   ...
   */
  const triggerSendMessage = async (overrideText = null) => {
 ...
 const handleReturnToTitle = () => {
  ...
  if (currentPage === '/saveload') {
    return <AppProvider value={contextValue}><div key="page-saveload" className="gwc-page-in"><SaveLoadPage /></div></AppProvider>;
  }
  if (currentPage === '/log') {
    return <AppProvider value={contextValue}><div key="page-log" className="gwc-page-in"><LogPage /></div></AppProvider>;
  }
 ...
 {/* ✨ 新增：全局备份与恢复进度条 (左上角悬浮) */}
      ...
      {toast.visible && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[99999] pointer-events-auto gwc-panel-in">
      ...
      {confirmDialog.isOpen && (
        <div className="fixed inset-0 z-[100000] bg-black/60 flex items-center justify-center backdrop-blur-sm pointer-events-auto gwc-overlay-in">
           <div className="bg-[#fdfaf5] text-[#4a4036] p-8 rounded-xl border-2 border-[#d9c5b2] shadow-[0_20px_60px_rgba(0,0,0,0.8)] max-w-sm w-full gwc-modal-in">
      ...
      {isMemoOpen && !visualAdjustMode && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-[100000] flex flex-col font-sans pointer-events-auto items-center justify-center gwc-overlay-in">
           <div className="bg-[#fdfaf5] w-[90%] max-w-2xl max-h-[85vh] rounded-2xl overflow-hidden flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.5)] border-2 border-[#d9c5b2] gwc-modal-in">
      ...
      {appMode === 'title' && (
        <div key="title-screen" className="absolute inset-0 z-20 pointer-events-none flex gwc-title-in">
          ...
          <div className="absolute bottom-6 right-8 text-white/60 font-bold text-sm drop-shadow-md pointer-events-none">v{import.meta.env.VITE_APP_VERSION}</div>
        </div>
      )}
    ...
    {appMode === 'game' && !visualAdjustMode && (
        <div key="game-screen" className="gwc-page-in">
        <>
          {!activePluginUI && (
     ...
     {/* SAVE / LOAD UI */}
      <Transition visible={isSaveLoadUIOpen && !visualAdjustMode}
        enterClass="gwc-overlay-in" exitClass="gwc-overlay-out"
        duration={210}>
        <div className="fixed inset-0 z-[100] flex flex-col font-sans select-none pointer-events-auto bg-gradient-to-b from-[#87CEEB] to-[#E0F6FF]">
          <div className="flex flex-col h-full gwc-panel-in">
          ...
          <div className="flex-1 min-h-0 px-4 md:px-12 py-2 md:py-4 flex flex-col w-full max-w-7xl mx-auto overflow-y-auto light-scrollbar">
             {slPage === 1 && (
              ...
              <div key={`slots-${slMode}-${slPage}`} className="grid grid-cols-2 gap-x-2 md:gap-x-8 gap-y-2 md:gap-y-4 flex-1 pb-4 gwc-panel-in">
      ...
      {isBotPanelOpen && getCurrentUser() === 'Admin' && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm pointer-events-auto gwc-overlay-in">
          <div className="bg-gradient-to-b from-[#1a2332] to-[#0f172a] rounded-2xl shadow-2xl w-[95vw] max-w-3xl max-h-[90vh] flex flex-col overflow-hidden border border-white/10 gwc-modal-in">
            {/* Header */}
      ...
      {/* Log 历史记录遮罩层 */}
      <Transition visible={isLogOpen && !visualAdjustMode}
        enterClass="gwc-overlay-in" exitClass="gwc-overlay-out"
        duration={210}>
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-40 flex flex-col font-sans pointer-events-auto">
          <div className="gwc-panel-in flex flex-col h-full">
