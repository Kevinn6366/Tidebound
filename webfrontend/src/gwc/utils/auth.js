...
function setRememberLogin(username) {
  localStorage.setItem(REMEMBER_KEY, JSON.stringify({ user: username }));
}
...
export async function registerUser(username, password, noPassword = false) {
  if (!username || username.length < 2) return { ok: false, msg: '用户名至少2个字符' };
  if (!noPassword && (!password || password.length < 4)) return { ok: false, msg: '密码至少4个字符' };

  try {
    const resp = await fetch(`${API_BASE}/api/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: username.trim(), password, no_password: noPassword })
    });
...
export function checkAutoLogin() {
  // 会话令牌必须存在才视为已登录（令牌过期后后端会拒绝，需重新登录）
  if (isLoggedIn() && getToken()) return true;
