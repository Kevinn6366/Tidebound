...
export const loadBatchCoreData = async (keys, includeMedia = false) => {
    ...
    // 刻意区分“加载失败”与“无数据”：无数据时后端返回 200 + {}；
    // 网络失败 / 401 / 403 / 5xx 时抛出 isLoadFailure，让调用方避免用空数据
    // 覆盖服务端（防止后端抖动或未登录导致聊天记录被清空）。
    let resp;
    try {
        resp = await fetch(`${API_BASE}/api/userdata/${mid}/batch?keys=${encodeURIComponent(keysStr)}${mediaParam}`);
    } catch (e) {
        const err = new Error('后端不可达'); err.isLoadFailure = true; throw err;
    }
    if (!resp.ok) {
        const err = new Error(`批量加载失败: ${resp.status}`);
        err.isLoadFailure = true; err.status = resp.status;
        throw err;
    }
