...
const SettingSlider = ({ label, value, min, max, step, suffix = '', onChange }) => (
  <div className="flex flex-col gap-2 w-full">
    ...
    <input aria-label={label} type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(parseFloat(e.target.value))} className="w-full h-2 bg-[#d9c5b2] rounded-lg appearance-none cursor-pointer accent-[#ba3f42]" />
  </div>
);
