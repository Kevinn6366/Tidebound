...
export default function SoundTab() {
  ...
  React.useEffect(() => () => {
    ...
    clearInterval(qwenPollRef.current);
  }, []);
  // 模型下载（独立于环境安装）
  ...
  /**
   * 读取 GPT-SoVITS 服务状态，失败时标为不可用。
   * @returns 服务状态；失败返回 null。
   */
  const fetchTtsStatus = React.useCallback(async () => {
    try {
      ...
      setTtsStatus(s);
      return s;
    } catch (e) { setTtsStatus({ ok: false, unavailable: true }); setTtsMsg(e.message); return null; }
  }, []);

  /**
   * 刷新 GPT-SoVITS 音色列表并显示请求错误。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const fetchVoices = React.useCallback(async () => {
    ...
    } catch (e) { setTtsVoices([]); setTtsMsg(e.message); }
  }, []);
  ...
  /**
   * 扫描可导入的 GPT-SoVITS 安装目录。
   * @param manual - 用户手动指定的安装根目录；省略时自动扫描。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const scanSources = async (manual) => {
    ...
    } catch (e) { setInstSources([]); setTtsMsg(e.message); }
    setInstScanning(false);
  };

  /**
   * 加载安装目录中的音色，默认选择各角色最新权重。
   * @param p - 待扫描的 GPT-SoVITS 根目录。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const loadInstVoices = async (p) => {
    ...
    } catch (e) { setInstVoices([]); setTtsMsg(e.message); }
  };

  /**
   * 请求导入选中的 GPT-SoVITS 音色并跟踪进度。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const startInstall = async () => {
    ...
    try {
      ...
      const timer = setInterval(async () => {
        ...
        } catch (e) { clearInterval(timer); setInstProg(null); setTtsMsg('安装进度读取失败: ' + e.message); }
      }, 1000);
    } catch (e) { setTtsMsg('安装失败: ' + e.message); }
  };
  ...
  const fetchQwenStatus = React.useCallback(async () => {
    try {
      ...
      setQwenStatus(s);
      return s;
    } catch (e) { setQwenStatus({ ok: false, unavailable: true }); setQwenMsg(e.message); return null; }
  }, []);

  /**
   * 刷新 Qwen3-TTS 音色列表并显示请求错误。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const fetchQwenVoices = React.useCallback(async () => {
    ...
    } catch (e) { setQwenVoices([]); setQwenMsg(e.message); }
  }, []);

  /**
   * 读取 Qwen3-TTS 支持的情绪配置。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  const fetchEmotions = React.useCallback(async () => {
    ...
    } catch (e) { setEmotions([]); setQwenMsg(e.message); }
  }, []);
  ...
  // ================= Qwen3-TTS 模型下载（独立） =================
  /**
   * 读取 Qwen3-TTS 环境和模型可用状态。
   * @returns 环境和模型状态；失败返回 null。
   */
  const fetchQwenModelStatus = React.useCallback(async () => {
    try {
      ...
      setQwenModel(d);
      return d;
    } catch (e) { setQwenMsg(e.message); return null; }
  }, []);

  /**
   * 读取 Qwen3-TTS 模型下载进度。
   * @returns 下载状态；失败返回 null。
   */
  ...
  const editQwenVoice = (v) => {
    ...
    // 滚动到表单
    document.getElementById('qwen-voice-add-form')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  };

  // ================= GPT-SoVITS 模型快速添加 =================
  /**
   * 提交 GPT-SoVITS 模型路径与参考素材。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  ...
  // ================= Qwen3-TTS 一键安装 =================
  /**
   * 读取 Qwen3-TTS 独立环境安装进度。
   * @returns 安装状态；失败返回 null。
   */
  ...
  // ================= torch 版本管理 + 检查更新 =================
  /**
   * 读取 PyTorch 当前版本和可选安装版本。
   * @returns 版本信息；失败返回 null。
   */
  ...
  /**
   * 按所选镜像请求切换 PyTorch 版本。
   * @param variant - 目标 PyTorch 加速平台版本。
   * @returns 无返回值；更新面板状态，失败显示错误。
   */
  ...
  /**
   * 读取 Qwen3-TTS 模型列表及当前选择。
   * @returns 模型列表和当前模型；失败返回 null。
   */
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* TTS 语音合成 */}
      <p role="note" className="text-xs text-[#7a6b5d]">配音设置已保留；Tidebound 尚未接入 TTS、硬件检测和模型管理服务。</p>
      ...
      <div className="bg-white/60 p-6 rounded-xl border border-[#e6d5b8] shadow-sm space-y-6">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 transition-opacity ${!settings.ttsEnabled && 'opacity-50 pointer-events-none'}`}>
          ...
          <div className="md:col-span-3 border-t border-dashed border-[#e6d5b8] pt-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select aria-label="配音引擎" value={settings.ttsEngine || 'sovits'} onChange={e => setSettings({ ...settings, ttsEngine: e.target.value })} className="bg-white border border-[#d9c5b2] text-[#4a4036] font-bold rounded-md px-3 py-1.5 text-xs outline-none shadow-inner">
            ...
            {settings.ttsBuiltIn && (settings.ttsEngine || 'sovits') === 'sovits' && (
              <div className="mb-5 bg-[#fdfaf5] p-5 rounded-xl border border-[#e6d5b8] shadow-inner space-y-4">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {ttsStatus === null ? (
                      ...
                      <span className="text-xs text-[#ba3f42]">服务不可用</span>
                    ) : ttsStatus.running ? (
                ...
                {ttsStatus && !ttsStatus.unavailable && !ttsStatus.installed && (
            ...
            {(settings.ttsEngine || 'sovits') === 'sovits' && (
            ...
            <p className="text-[11px] text-[#7a6b5d] mt-2">{(settings.ttsEngine || 'sovits') === 'sovits' ? '留空则不传参考音频参数，由服务端使用自身配置的默认音色。' : 'Qwen3-TTS 音色与情绪在上述「内置配音服务 (Qwen3-TTS)」控制台配置，无需此处参考音频参数。'}</p>
          </div>
        </div>
      </div>
      ...
      {showDoc && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowDoc(null)}>
          <div className="bg-white rounded-2xl border border-[#e6d5b8] shadow-2xl p-6 w-[min(95vw,720px)] max-h-[88vh] overflow-y-auto space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-[#ba3f42] mb-4">以下为上游 GWC-Pro 配置参考，其中的目录和安装操作不适用于当前 Tidebound。</p>
            {showDoc === 'sovits' ? (
