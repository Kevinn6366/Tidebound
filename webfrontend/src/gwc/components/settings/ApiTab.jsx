...
export default function ApiTab() {
  ...
  const [ocAutostartMsg, setOcAutostartMsg] = useState('');
  useEffect(() => {
    fetch('/api/launcher-config')
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d) setDisableOcAutostart(!!d.disableOpencodeAutostart); })
      .catch(() => {});
  }, []);
  const saveOcAutostart = async (v) => {
    setDisableOcAutostart(v);           // 乐观更新，避免开关卡顿
    setOcAutostartMsg('');
    try {
      const r = await fetch('/api/launcher-config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ disableOpencodeAutostart: v })
      });
      if (!r.ok) throw new Error('保存失败');
      setOcAutostartMsg('已保存，下次启动生效。');
    } catch (e) {
      setDisableOcAutostart(!v);        // 回滚，避免显示与实际不符
      setOcAutostartMsg('保存失败，请检查后端是否运行。');
    }
  };
      ...
      <div className="bg-white/60 p-6 rounded-xl border border-[#e6d5b8] shadow-sm space-y-6">
        ...
        <div className="pt-6 border-t-2 border-[#e6d5b8]">
          ...
          <div className="mt-4 pt-4 border-t border-dashed border-[#e6d5b8]">
            <SettingToggle
              label="禁用 OpenCode CLI 跟随启动"
              value={!!disableOcAutostart}
              onChange={saveOcAutostart}
            />
            <p className="text-xs text-[#a89578] mt-2 leading-relaxed">
              开启后，运行「启动全栈环境.bat」时将不再自动检测安装、也不再自动打开 OpenCode 命令行窗口。
              仍可通过桌宠托盘菜单的「打开 OpenCode」手动启动。
