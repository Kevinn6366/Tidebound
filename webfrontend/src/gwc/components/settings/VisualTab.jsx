...
import { Eye, Upload, Trash2, RefreshCw, Image as ImageIcon, Monitor, Database, Sun, Moon, Laptop } from 'lucide-react';
import { getThemeMode, setThemeMode, resolveTheme, watchSystemTheme } from '../../utils/theme';

/** @returns {import("react").ReactElement} 原视觉设置及带明确标签的资源导入控件。 */
export default function VisualTab() {
  ...
  // 主题：浅色 / 深色 / 跟随系统。主题存于 localStorage（与账号数据无关，属本机偏好）
  const [themeMode, setThemeModeState] = React.useState(() => getThemeMode());
  const [actualTheme, setActualTheme] = React.useState(() => resolveTheme());
  React.useEffect(() => {
    // 处于「跟随系统」时，系统切换深浅色要实时生效
    const stop = watchSystemTheme((actual) => setActualTheme(actual));
    return stop;
  }, []);
  const changeTheme = (mode) => {
    const actual = setThemeMode(mode);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* 外观主题 */}
      <SettingSectionTitle title="外观主题 (Appearance)" />
      <div className="bg-white/60 p-5 rounded-xl border border-[#e6d5b8] shadow-sm mb-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <p className="text-sm font-bold text-[#4a4036]">界面配色</p>
            <p className="text-xs text-[#7a6b5d] mt-1">
              当前生效：<span className="font-bold">{actualTheme === 'dark' ? '深色' : '浅色'}</span>
              {themeMode === 'system' && '（由系统设置决定）'}
            </p>
          </div>
          <div className="flex items-center gap-1 p-1 rounded-full bg-[#e8decb] border border-[#e6d5b8] shrink-0">
            {THEME_OPTIONS.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${
                  themeMode === id
                    ? 'bg-[#4fa0d8] text-white shadow-sm'
                    : 'text-[#7a6b5d] hover:bg-white/60'
                }`}
                title={id === 'system' ? '自动跟随操作系统的深色/浅色设置' : `固定使用${label}模式`}
              >
                <Icon size={14} /> {label}
              </button>
            ))}
        ...
        <p className="text-xs text-[#7a6b5d] mt-3 leading-relaxed">
          深色模式会同时作用于主界面、设置面板以及内置的剧情IDE、立绘模式等模块。
          选择「跟随系统」后，系统在深浅色之间切换时界面会实时同步。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white/60 p-5 rounded-xl border border-[#e6d5b8] shadow-sm">
          ...
          <input aria-label="导入游戏背景图" type="file" accept="image/*" multiple onChange={handleBgUpload} className="block w-full text-sm text-[#7a6b5d] file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-[#8fbf8f] file:text-white hover:file:bg-[#7ebd7e] cursor-pointer mb-4"/>
          {bgList.length > 0 && (
            <div className="max-h-32 overflow-y-auto bg-white rounded-lg p-2 border border-[#e6d5b8] space-y-1">
              {bgList.map(bg => (
        ...
        <div className="bg-white/60 p-5 rounded-xl border border-[#e6d5b8] shadow-sm">
          ...
          <input aria-label="导入标题背景图" type="file" accept="image/*" onChange={handleTitleBgUpload} className="block w-full text-sm text-[#7a6b5d] file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-[#4fa0d8] file:text-white hover:file:bg-[#5db4f0] cursor-pointer"/>
          {localTitleBgImage && (
      ...
      {/* 动画速率调节 */}
      <SettingSectionTitle title="动画速率 (Animation Speed)" />
      <div className="bg-white/60 p-6 rounded-xl border border-[#e6d5b8] shadow-sm space-y-5">
        <SettingSlider
          label="全局动画速率"
          value={settings.animationRate ?? 1}
          min={0.25} max={3} step={0.25} suffix="x"
          onChange={v => setSettings({...settings, animationRate: v})}
        />
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className="text-xs text-[#7a6b5d] flex-1 min-w-[200px]">
            调节所有页面切换 / 弹窗过渡动画的速度。0.25x 最慢（更从容），3x 最快（更利落），1x 为默认。
          </p>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-[#a89578]">预设：</span>
            {[['慢速', 0.5], ['默认', 1], ['快速', 2]].map(([label, v]) => (
              <button key={label} onClick={() => setSettings({...settings, animationRate: v})}
                className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${(settings.animationRate ?? 1) === v ? 'bg-[#5ab4ed] text-white' : 'bg-[#e8decb] text-[#4a4036] hover:bg-[#d9c5b2]'}`}>
                {label} {v}x
              </button>
            ))}
          </div>
