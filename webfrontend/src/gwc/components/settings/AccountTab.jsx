...
export default function AccountTab() {
  ...
  const [localModeMsg, setLocalModeMsg] = useState(null);

  useEffect(() => {
    fetch(`${API}/api/local-mode`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d && typeof d.localMode === 'boolean') setLocalMode(d.localMode); })
      .catch(() => {});
  }, []);
  ...
  const handleToggleLocalMode = async () => {
    ...
    setLocalModeMsg(null);
    try {
      const resp = await fetch(`${API}/api/local-mode`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ localMode: next })
      });
      const data = await resp.json();
      if (data.ok) {
        setLocalMode(data.localMode);
        setLocalModeMsg({ ok: true, text: data.localMode ? '本地模式已开启：登录任何账号无需密码' : '本地模式已关闭' });
      } else {
        setLocalModeMsg({ ok: false, text: data.msg || '操作失败' });
      }
    } catch (e) {
      setLocalModeMsg({ ok: false, text: '操作失败' });
    } finally {
      ...
      <div className="bg-white rounded-2xl border border-[#e6d5b8] shadow-sm p-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="text-sm font-bold text-[#4a4036]">本地模式</p>
            <p className="text-xs text-[#7a6b5d] mt-1">
              开启后登录任何账号无需密码，并自动隐藏登录页的密码输入框；登录状态持续到手动退出登录。
            </p>
          </div>
          <button
            ...
            style={{ background: localMode ? '#5ab4ed' : '#cbd5e1' }}
            title={localMode ? '点击关闭本地模式' : '点击开启本地模式'}
          >
            <span
              className="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform"
              style={{ transform: localMode ? 'translateX(24px)' : 'translateX(0)' }}
            />
          </button>
        </div>
        {localModeMsg && (
          <div className={`flex items-center gap-2 p-3 rounded-lg text-sm mt-3 ${localModeMsg.ok ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
            {localModeMsg.ok ? <CheckCircle size={14} /> : <AlertCircle size={14} />}
            {localModeMsg.text}
          </div>
        )}
      </div>
